...
public class ProducerDemo {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 10_000; i++) {
			if (isAsync) { // 发送异步消息
				producer.send(new ProducerRecord<String, String>("apt-test", Integer.toString(i),
						Integer.toString(i)), new DemoCallBack(System.currentTimeMillis(), i,
								Integer.toString(i)));
			} else { // 发送同步消息
				try {
					producer.send(
							new ProducerRecord<String, String>("apt-test", Integer.toString(i),
